...
class TextSample:
    def __init__(self, text, author):
        ...
        self.word_count_frequency = build_frequency_table(ngram_creator(preapre_text(text)))
    def __str__(self):
        return "Author's Name: {} and Average Sentence Length : {}".format(self.author, self.average_sentence_length)
...
print(myrtle_sample)
gregg_sample = Text Sample(gregg_t_fishy_intro, "Gregg T Fishy")
print(gregg_sample)
...
def build_frequency_table(corpus):
    ...
    for word in unique_words:
        ...
        counter = 0
    frequnecy_table = dict(zip(unique_words, word_count))
...
def ngram_creator(text_list):
    ...
    for counter in range(0, len(text_list)):
        counter += 1
        if counter % 2 != 0:
            two_word_n_gram.append((text_list[counter-1]+ " " + text_list[counter]))
    return two_word_n_gram
...
def frequency_comparision (table1, table2):
    appearances = 0
...
def percent_difference(value1, value2):
    value = abs(value 1 - value2) / ((value1 + value2)/2)
    return value
...
def find_text_similarity(text1, text 2):
    sentence_length_difference = percent_difference(get_average_sentence_length(text1), get_average_sentence_length(text2))
    setence_length_similarity = abs(1-float(sentence_length_difference))
    word_count_similarity = frequency_comparision(build_frequency_table(prepare_text(text1)), build_frequency_table(prepare_text(text2)))
...
print ("Myrtle Beech's score: {}".format(find_text_similarity(myrtle_beech_intro, murder_note)))
print ("Lily Trebuchet's score: {}".format(find_text_similarity(lily_trebuchet_intro, murder_note)))
print ("Gregg T Fishy's score: {}".format(find_text_similarity(gregg_t_fishy_intro, murder_note)))
